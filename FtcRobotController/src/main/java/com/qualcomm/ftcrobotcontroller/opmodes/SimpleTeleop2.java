...
public class SimpleTeleop2 extends OpMode
{
  ...
  */
    ...
    public void loop() {
        ...
        /*Gamepad 1:
        -drive the robot
        ***Will happen through the use of the joystick
        -ability to control the direction of the carwash
                ***will use y for in, will use a for shooting out
       */
        float power = -gamepad1.left_stick_y; //Foward, Backwards
        float direction = gamepad1.left_stick_x; // Left, Right
        float right = power - direction;
        ...
        -control the arm extension
                ***will use y for out, will use a to pull
        -control the arm angle
                ***will use d-pad up for retraction, will use d-pad down for going down
        -control the door for scoring
                ***will use b to release contained object, will use x to return back to place
        -control the servo for knocking down the climbers
        ***will use the right bumper to extend and retract
        */
        if(gamepad2.a)
        {
        // make the arm move up
            armangle=30;
            motorangle.setPower(armangle);
        }
        if(gamepad2.y)
        {
        // make the arm move down
            armangle=-30;
            motorangle.setPower(armangle);
        }
        if(gamepad2.right_bumper)
        {
        //servo opens for dropping the blocks
        }
        if(gamepad2.left_bumper)
        {
        //servo moves to knock down the climbers
        }
        if(gamepad2.)
        {
        //arm extends outward
            armextent=30;
            motorextension.setPower(armextent);
        }
        if(gamepad2.)
        {
        //arm retracts in to pull oursleves up
            armextent=-30;
            motorextension.setPower(armextent);
        }
		/*
